...
from .transmonChain import TransmonChain
...
class ChainTwoToneSimulation:
    ...
    def set_grid(self, freqs, currs=None, Omegas = None):
        ...
        if self._currs is not None:
            ...
            self._params = np.array(self._phis).T
    ...
    def generate_caches(self):
        ...
        for freq in self._freqs:
            self._chain.set_omega(2*np.pi*freq)
            self._chain.build_RF_subtrahend()
        print(".", end="")

        self._chain.set_omega(2 * np.pi * self._freqs[0])
    ...
    def _iteration(self, param_ids):
        ...
        try:
            ans = qp.steadystate(H, self._c_ops, **self._steady_kwargs)
            ...
            kwargs["use_rcm"]=True
            ans = qp.steadystate(H, self._c_ops, **kwargs)
    ...
    def run(self, n_cpus):
        ...
        with Pool(n_cpus) as p:
            ...
            self.spec = np.reshape(np.array([result[0] for result in results], dtype=object), (len(self._params), len(self._freqs)))
