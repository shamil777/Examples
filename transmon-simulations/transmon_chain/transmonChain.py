from ..single_transmon.transmon import Transmon
from qutip import *
...
class TransmonChain:
    def __init__(self, length, transmon_truncation=2,
        ...
        self._phi = []
        self._c_ops = None
        self._transmons: list[Transmon] = [None] * self._length
    ...
    def build_low_energy_kets(self, total_population, max_simultaneously_above_first_excited):
        ...
        for idx, state_combination in enumerate(transmon_states):
            state_combination = np.array(state_combination)
            if sum(state_combination) <= total_population:
                if len(np.where(state_combination >= 2)[0]) <= max_simultaneously_above_first_excited:
                    self._low_energy_states.append(state_combination)
                    self._low_energy_state_indices.append(idx)
           
        self._low_energy_states_mask = np.zeros([idx+1]*2, dtype=bool)
